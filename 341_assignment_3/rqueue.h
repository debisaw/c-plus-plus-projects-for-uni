...
private:
  ...
  Node* recursiveMerge(Node *heap1, Node* heap2);
  void recursivePreOrderPrint(Node* node) const ;
  void recursiveInsertForSetPri(RQueue& newQueue, Node* node);
  Node*  makeDeepCopy(const Node* rhsNode);
};
#endif
