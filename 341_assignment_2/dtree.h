...
public:
  ...
  //    void rebalance(DNode*& node);
    ...
    //----------------
  bool checkRemove(int disc, DNode*& removed);
private:
  DNode* _root;
  /* IMPLEMENT (optional): any additional helper functions here */
  bool recursiveInsert(DNode* currentNode,Account newAcct);
  void recursiveClear(DNode* node);
  DNode* recursiveRetrieve(int disc, DNode* node);
  bool removeHelper(int disc);
  //  bool checkRemove(int disc, DNode*& removed);
  void recursivePrintAccounts(DNode* node) const;
  bool checkImbalanceHelper();
  bool findImbalance(DNode* node);
  int helperMakeArray(DNode* node,DNode* array[],int i);
  DNode* makeArrayToBST(DNode* array[] , int min, int max);
  void removeVacant(DNode* node);
  void  preOrderInsertAccount(DNode* node);
  void preOrderInsertData(DNode* newNode,DNode* oldNode);
};
